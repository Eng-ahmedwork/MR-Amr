...
function initTheme() {
    ...
    themeBtn?.addEventListener('click', () => {
        const currentTheme = root.getAttribute('data-theme') || body.getAttribute('data-theme') || 'dark';
        ...
        <div class="table-responsive">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    <td style="padding: 1rem; display: flex; gap: 0.5rem; justify-content: flex-start; flex-wrap: wrap;">
                                ...
                                <button onclick="clearStudentHistory('${s.id}')" class="btn-action btn-clear"><i class="fas fa-eraser"></i> Clear</button>
...
async function clearStudentHistory(id) {
    const student = students.find(s => s.id === id);
    if (!student) return;

    const confirmed = await Modal.confirm(
        'Clear Student History',
        `Are you sure you want to clear all history (Attendance, Grades, Fees & Report logs) for ${student.name}?\n\nBasic student information will be preserved.\nThis action cannot be undone.`
    );
    ...
    if (confirmed) {
        ...
        await Modal.alert('Cleared', `History has been successfully cleared for ${student.name}.`);
    }
}
        ...
        <div class="grid-3" style="align-items: start;">
            <div class="card" style="text-align: center; position: sticky; top: 1rem; padding: 1rem;">
                ...
                <div style="position: absolute; top: 10px; right: 10px; display: flex; gap: 8px;">
                    <button onclick="clearStudentHistory('${student.id}')" title="Clear History" style="background: none; border: none; color: var(--text-muted); cursor: pointer; font-size: 1.1rem; transition: color 0.2s;" onmouseover="this.style.color='#a855f7'" onmouseout="this.style.color='var(--text-muted)'">
